...
namespace CiftlikTakip.MvcUI.Areas.admin.Controllers
{
    [Area("Admin")]
    [SessionAspect]
    public class AnimalTreatmentController : Controller
    {
        private readonly IHttpApiService _httpApiService;
        public AnimalTreatmentController(IHttpApiService httpApiService)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                var httpResponseMessage = await client.GetAsync($"{client.BaseAddress}/AnimalTreatments");

                var jsonResponse = await httpResponseMessage.Content.ReadAsStringAsync();

                var response = JsonSerializer.Deserialize<ResponseBody<List<AnimalTreatmentItem>>>(jsonResponse, new JsonSerializerOptions() { PropertyNameCaseInsensitive = true });
