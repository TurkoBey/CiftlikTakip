...
namespace Infrastructure.DataAccess.Implementations.EF
{
    ...
    where TContext : DbContext, new()
    {
        ...
        public async Task<TEntity> Get2Async(Expression<Func<TEntity, bool>>? predicate = null, params Expression<Func<TEntity, object>>[] includeList)
        {
            using (var ctx = new TContext())
            {
                IQueryable<TEntity> dbSet = ctx.Set<TEntity>();
        ...
        public async Task<List<TEntity>> GetAll2Async(Expression<Func<TEntity, bool>>? predicate = null, params Expression<Func<TEntity, object>>[] includeList)
        {
            using (var ctx = new TContext())
            {
                IQueryable<TEntity> dbSet = ctx.Set<TEntity>();
