...
{
    internal class AnimalTreatmentBs : IAnimalTreatmentBs
    {
        private readonly IAnimalTreatmentRepository _repo;
        private readonly IMapper _mapper;

        public AnimalTreatmentBs(IAnimalTreatmentRepository repo, IMapper mapper)
        {
            _repo = repo;
            _mapper = mapper;
        }
        public async Task<ApiResponse<AnimalTreatment>> InsertAsync(AnimalTreatmentPostDto dto)
        {
            if (ValidationHelper<Guid>.IsGuid(dto.Id))
            {
                if (dto != null)
                {
                    ...
                    dto.IsActive = true;
                    var taskAnimalTreatment = _mapper.Map<AnimalTreatment>(dto);
                    var insertedAnimalTreatment = await _repo.InsertAsync(taskAnimalTreatment);
                    return ApiResponse<AnimalTreatment>.Success(StatusCodes.Status201Created, insertedAnimalTreatment);
        ...
        public async Task<ApiResponse<NoData>> UpdateAsync(AnimalTreatmentPutDto dto)
        {
            if (ValidationHelper<Guid>.IsGuid(dto.Id))
            {
                ...
                if (itemId != null)
                {
                    if (!StructuralComparisons.StructuralEqualityComparer.Equals(dto.Version, ConvertorHelper.ByteArrayToString(itemId.Version)))
                    {
                        ...
                        dto.Version = BaseBs.GenerateVersionNumber();
                        dto.IsActive = true;
                        var updatetask = _mapper.Map<AnimalTreatment>(dto);
        ...
        public async Task<ApiResponse<AnimalTreatmentGetDto>> GetByIdAsync(Guid id, params string[] includeList)
        {
            if (ValidationHelper<Guid>.IsGuid(id))
            {
                ...
                if (result != null)
                {
                    var dto = _mapper.Map<AnimalTreatmentGetDto>(result);
                    return ApiResponse<AnimalTreatmentGetDto>.Success(StatusCodes.Status200OK, dto);
                }
                throw new NotFoundException("Aradığınız Id'ye uygun veri bulunamadı.");
            }
            throw new BadRequestException("Id değeri hatalı,");
        }
        public async Task<ApiResponse<List<AnimalTreatmentGetDto>>> GetTreatmentsAsync(params string[] includeList)
        {
            var result = await _repo.GetAllAsync(x => x.IsActive == true, includeList: includeList);
            if (result.Count > 0)
            {
                var returnList = _mapper.Map<List<AnimalTreatmentGetDto>>(result);
                return ApiResponse<List<AnimalTreatmentGetDto>>.Success(200, returnList);
            }
